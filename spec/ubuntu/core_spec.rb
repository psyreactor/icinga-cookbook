...
describe 'icinga::core' do
  ...
  it 'install packages required for icinga core' do
    ...
    expect(chef_run).to install_package('libjpeg8')
    expect(chef_run).to install_package('libjpeg8-dev')
    expect(chef_run).to install_package('libpng12-0')
    expect(chef_run).to install_package('libpng12-dev')
    expect(chef_run).to install_package('libdbi1')
    expect(chef_run).to install_package('libdbi-dev')
    expect(chef_run).to install_package('libdbd-mysql')
    expect(chef_run).to install_package('snmp')
    expect(chef_run).to install_package('libsnmp-dev')
  end
  ...
  it 'create icinga apache site' do
    expect(chef_run).to create_template('/etc/apache2/sites-available/icinga.conf')
  end
